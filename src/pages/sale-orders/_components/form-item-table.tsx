...
export function FormItemTable({ items, products, onRemoveItem }: FormItemTableProps) {
  if (items.length === 0) {
    return (
          ...
          {items.map(item => {
            ...
            return (
              <tr key={item.productId} className="bg-card">
                <td className="px-4 py-3 font-medium text-foreground">{product?.name ?? item.productId}</td>
                <td className="px-4 py-3 text-center">
                  <div className="inline-flex items-center justify-center bg-muted px-2.5 py-0.5 rounded-md font-medium text-foreground">
                    {item.qty}
                  </div>
                </td>
